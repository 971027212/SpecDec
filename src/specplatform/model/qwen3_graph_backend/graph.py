...
from specplatform.model.qwen3_graph_backend.cache import KVCache
...
class GraphEngine:
    ...
    def forward(
        ...
        with self._lock:
            ...
            attention_mask = _invert_mask(attention_mask)
...
def _invert_mask(mask: torch.Tensor) -> torch.Tensor:
    """Convert an allow mask with 1/0 entries to additive attention mask."""
    if mask.min() == 0:
